...
onready var text = get_parent().get_node("Dialogo").dialog7
...
func _ready():
	...
	$HumorColor.color = Color(convertColorR(Global.facundesMood), convertColorG(Global.facundesMood), 0)
	$HumorColor.rect_size.y = convertSize(Global.facundesMood)
	#Chama a função que carrega o dialogo
	load_dialogo()
	
	if Global.facundesMood > 7:
		$EmojiNeutro.hide()
		$EmojiBravo.hide()
		$SrFacundesNpcDefinitivoBravo.hide()
		$SrFacundesNpcDefinitivoNeutro.hide()
	if Global.facundesMood <= 7 and Global.facundesMood >= 3:
		$SrFacundesNpcDefinitivoBravo.hide()
		$SrFacundesNpcDefinitivoNeutro.show()
		$EmojiBravo.hide()
		$EmojiNeutro.show()
	if Global.facundesMood < 3:
		$EmojiBravo.show()
...
func _physics_process(delta):
	...
	if btnClicked :
		HumSizeAnim.interpolate_property(
			...
			$HumorColor.rect_size, Vector2(56, convertSize(Global.facundesMood)), 1,
		...
		HumColorAnim.interpolate_property(
			$HumorColor, "color",
			$HumorColor.color, Color(convertColorR(Global.facundesMood), convertColorG(Global.facundesMood), 0), 1,
			Tween.TRANS_LINEAR, Tween.EASE_IN_OUT
		)
		HumColorAnim.start()
...
func _on_Btn_A_pressed() -> void:
	...
	if a == 0:
		Global.assertive  +=1
		if Global.facundesMood <= 8:
			Global.facundesMood += 2
	elif a == 1:
		Global.passive +=1
		Global.facundesMood -= 2
	else:
		Global.passiveAgressive +=1
		Global.facundesMood -= 3
	btnClicked = true
	haveChoices = false
	...
	if Global.facundesMood + Global.facundesMood <= 0: loose = true
...
#Função executada ao pressionar o botão Simon
func _on_Simon_pressed() -> void:
	#Leva o jogador para a cena do Simon
