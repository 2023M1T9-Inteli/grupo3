...
onready var text = get_parent().get_node("Dialogo").dialogo_5
...
func load_dialogo(res, resIndex):
	...
	if dialogo_index < text.size() and perdeu == false:
		...
		#Exiibe o texto gradualmente
		$TextBox/Tween.interpolate_property(
			$TextBox/RichTextLabel, "percent_visible", 0, 1, 5,
			Tween.TRANS_LINEAR, Tween.EASE_IN_OUT
		)
		#Inicia a animação
		$TextBox/Tween.start()
...
func _on_Leticia_pressed() -> void:
	#Leva o jogador para a cena do Victor
	get_tree().change_scene_to(load("res://src/Leticia/Cena4Leticia.tscn"))
