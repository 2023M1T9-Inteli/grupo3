...
onready var text = get_parent().get_node("Dialogo").dialog11
...
func _ready():
	...
	$HumorColor.color = Color(convertColorR(Global.victorMood), convertColorG(Global.victorMood), 0)
	$HumorColor.rect_size.y = convertSize(Global.victorMood)
	#Chama a função que carrega o dialogo
	load_dialogo()
	
	if Global.victorMood > 7:
		$EmojiNeutro.hide()
		$EmojiBravo.hide()
		$VictorNpcDefinitivoBravo.hide()
		$VictorNpcDefinitivoNeutro.hide()
	if Global.victorMood <= 7 and Global.victorMood >= 3:
		$VictorNpcDefinitivoBravo.hide()
		$VictorNpcDefinitivoNeutro.show()
		$EmojiBravo.hide()
		$EmojiNeutro.show()
	if Global.victorMood < 3:
		$EmojiBravo.show()
...
func _physics_process(delta):
	...
	if btnClicked :
		HumSizeAnim.interpolate_property(
			...
			$HumorColor.rect_size, Vector2(56, convertSize(Global.victorMood)), 1,
		...
		HumColorAnim.interpolate_property(
			$HumorColor, "color",
			$HumorColor.color, Color(convertColorR(Global.victorMood), convertColorG(Global.victorMood), 0), 1,
			Tween.TRANS_LINEAR, Tween.EASE_IN_OUT
		)
		HumColorAnim.start()
...
func load_dialogo(resIndex = 0):
	...
	if dialogIndex < text.size() and loose == false:
		...
		$TextBox/RichTextLabel.bbcode_text = text[dialogIndex]["Text"][resIndex]
...
func _on_Btn_A_pressed() -> void:
	...
	if text[dialogIndex]["Text"].size() > 1:
		load_dialogo(a)
	else:
		load_dialogo()
