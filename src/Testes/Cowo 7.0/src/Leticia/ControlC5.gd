...
onready var text = get_parent().get_node("Dialogo").dialogo_6
...
func _process(delta: float) -> void:
	if Input.is_action_just_pressed("ui_up"):
		get_tree().change_scene_to(load("res://src/Leticia/Feedback.tscn"))
...
func _on_Relatrio_pressed() -> void:
	get_tree().change_scene_to(load("res://src/Minigames/FeedbackMinigame.tscn"))
