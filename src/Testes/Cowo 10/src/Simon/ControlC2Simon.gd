...
onready var text = get_parent().get_node("Dialogo").dialogo_9
...
#Função executada ao pressionar o botão Cutscene
func _on_Cutscene_pressed() -> void:
	#Leva o jogador para a cena do Cutscene
	get_tree().change_scene_to(load("res://src/Minigames/Cutscene.tscn"))
