...
onready var text = get_parent().get_node("Dialogo").dialogo_11
...
func _ready():
	...
	$HumorColor.color = Color(convertColorR(Global.VictorHumor), convertColorG(Global.VictorHumor), 0)
	$HumorColor.rect_size.y = convertSize(Global.VictorHumor)
	#Chama a função que carrega o dialogo
	load_dialogo()
	
	if Global.VictorHumor > 7:
		$EmojiNeutro.hide()
		$EmojiBravo.hide()
		$VictorNpcDefinitivoBravo.hide()
		$VictorNpcDefinitivoNeutro.hide()
	if Global.VictorHumor <= 7 and Global.VictorHumor >= 3:
		$VictorNpcDefinitivoBravo.hide()
		$VictorNpcDefinitivoNeutro.show()
		$EmojiBravo.hide()
		$EmojiNeutro.show()
	if Global.VictorHumor < 3:
		$EmojiBravo.show()
		$VictorNpcDefinitivoBravo.show()
...
func _physics_process(delta):
	...
	if btnClicked :
		HumSizeAnim.interpolate_property(
			...
			$HumorColor.rect_size, Vector2(56, convertSize(Global.VictorHumor)), 1,
		...
		HumColorAnim.interpolate_property(
			$HumorColor, "color",
			$HumorColor.color, Color(convertColorR(Global.VictorHumor), convertColorG(Global.VictorHumor), 0), 1,
			Tween.TRANS_LINEAR, Tween.EASE_IN_OUT
		)
		HumColorAnim.start()
...
func load_dialogo(resIndex = 0):
	...
	if dialogo_index < text.size() and perdeu == false:
		...
		$TextBox/RichTextLabel.bbcode_text = text[dialogo_index]["Text"][resIndex]
...
func _on_Btn_A_pressed() -> void:
	...
	if a == 0:
		Global.Ativa +=1
		if Global.VictorHumor <= 8:
			Global.VictorHumor += 2
	elif a == 1:
		Global.Passiva +=1
		Global.VictorHumor -= 0
	else:
		Global.PassivaAgressiva +=1
		Global.VictorHumor -= 3
	btnClicked = true
	haveChoices = false
...
#Função executada ao pressionar o botão Leticia
func _on_Facundes_pressed() -> void:
	#Leva o jogador para a cena do Victor
	get_tree().change_scene_to(load("res://src/Facundes/Cena2Facundes.tscn"))
