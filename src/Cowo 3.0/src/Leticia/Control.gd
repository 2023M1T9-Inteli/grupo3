...
#Booleana que armazena a condição que mostra se o tutorial está sendo exibido
var tutorial = true
...
func _physics_process(delta):
	...
	#caso o tutorial esteja sendo exibido e o jogador pressione espaço
	if tutorial and Input.is_action_just_pressed("ui_accept"):
		$Tutorial.hide()
		tutorial = false
		return
	
	#Caso a animação de texto esteja em andamento e não haja opcões, ou tutorial sendo exibido e a tecla espaço seja pressionada 
	if active and Input.is_action_just_pressed("ui_accept") and not haveChoices and not tutorial:
...
#Função executada ao pressionar o botão Victor
